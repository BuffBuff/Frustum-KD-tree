...
void main(uint3 threadID : SV_DispatchThreadID)
{
	...
	{ //nrOfTriangles
		if (Indices[mod] > 0)
		{
			if (triangles[Indices[mod]].pos0.x > 0)
			{
				outColor.x = 1;
			}
			else if (triangles[Indices[mod]].pos0.y > 0)
			{
				outColor.y = 1;
			}
			else if (triangles[Indices[mod]].pos0.z > 0)
			{
				outColor.z = 1;
			}
		}
	...
	//hit.x = Indices[index];
	//hit.x = triangles[index].ID;


	//super mega awesome iteration of doom and destruction!
	//if (RayVSAABB(r, KDtree[0].aabb) == MAXDIST) // missed box
	//{
